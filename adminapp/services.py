...
def get_order_by_user(id):
    with closing(connection.cursor()) as cursor:
        cursor.execute(""" SELECT food_order.id, food_customer.first_name,food_customer.last_name, food_order.address, food_order.payment_type,food_order.status,food_order.created from food_order 
...
def get_product_by_order(id):
    with closing(connection.cursor()) as cursor:
        ...
        food_orderproduct.created,food_product.name from food_orderproduct 
...
def get_table():
    with closing(connection.cursor()) as cursor:
        cursor.execute("""SELECT food_orderproduct.product_id, COUNT(food_orderproduct.product_id),food_product.name 
